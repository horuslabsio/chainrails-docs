...
- **`usePaymentModal`**: Manual session token control for advanced use cases
 ...
 <Note>
  For both cases, the `<PaymentModal />` component requires a server-generated session token to function. We've provided a Github repository with a curated Github template to help you get started quickly: <a href="https://github.com/horuslabsio/chainrails-session-server-demo">Chainrails Demo Server</a>
</Note>
---
...
- `styles` (object, optional): Available styling (e.g., `{ accentColor: "#FF6B6B" }`)
...
function App() {
  ...
  return (
    <div>
      ...
      <PaymentModal {...cr} styles={{ accentColor: "#FF6B6B" }} />
    </div>
  );
}
...
function App() {
  ...
  return (
    <div>
      ...
      <PaymentModal {...cr} styles={{ accentColor: "#FF6B6B" }} amount={amount} />
    </div>
  );
}
...
| `styles`       | object   | Custom styling object with options like `accentColor` |
