...
<Note>
  If you need a broader overview of the session flow, see the [Quickstart](/quickstart) and the [Payment Modal reference](/sdk-reference/components/payment-modal).
</Note>
...
1. **Test on real devices**: Wallet handoff behavior can differ from simulators
2. **Resume gracefully**: Make sure your app can recover when users return from a wallet app
4. **Log session failures**: Mobile network issues are common, so capture enough detail to debug failed session creation
