...
- If yu need a single quote for a specific route, we recommend using the routing engine's `findOptimalRoute` to retrieve the best quote. See [Finding the Optimal Route](/guides/routes) for more details.
- Otherwise, we recommend using the multi-source quote endpoint to get quotes from multiple source chains for a given destination chain. This fetches the best quotes for multiple source chains, providing more options for the user. See [Multi-Source Quotes](/api-reference/quotes/get-quotes-from-all-source-chains-to-destination) for more details.
...
1. Starts the intent automatically (startIntent)
...
**NB:** You can however trigger the intent processing flow manually, if the indexer goes down for any reason or you rather prefer to do that using the `GET /api/v1/intents/{intent_address}/trigger-processing` endpoint.
