...
<Note>
  For a complete list of supported networks and tokens, see [Supported Networks & Tokens](/essentials/integrations). Testnet networks are usuallly suffixed with `_TESTNET`.
</Note>
...
export async function POST(req) {
  const { destinationChain, token, amount, recipient } = await req.json();
...
function App() {
  const cr = usePaymentSession({
    ...
    amount: 1, // Set to 0, to let users input their own amount
    onCancel: () => {
...
- `onCancel`: Callback when user cancels the payment
